...
# Requires: gh CLI logged in, both APKs built. ASCII-only (PS 5.1 reads
# BOM-less files as ANSI); forward slashes only (JS/PS escaping trap).
...
if (-not $exists) {
  gh release create $tag -R railgun0325/dsh-phone --title 'v0.2.0 - one-tap Root + Shizuku editions' --notes-file $notes $rootRelease $shizukuRelease (Join-Path $stage 'SHA256SUMS-v0.2.0.txt')
} else {

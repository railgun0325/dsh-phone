...
4. 验证用 **最薄的 surface**：`dsh --profile headless 'Reply with exactly: upgrade-ok'`。
   这样一行就能看出轮次是否真的跑完，不用起 web、不用发 HTTP。
...
1. **先跑最薄的 surface**：`dsh --profile headless 'hi'`。它能一秒暴露 boot/轮次类异常
   （例如 `dsh: rename is not defined`），而 web 模式会把它吞进静默的 promise，看起来像"卡住"。

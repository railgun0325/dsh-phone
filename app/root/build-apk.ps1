...
foreach ($s in @('setup-root.sh','start-dsh.sh','boot-dsh.sh','dns-fwd.mjs','patch-dsh.mjs','patch-dsh-link.mjs','install-api-key.sh','cordis.patch.yml')) {
...
& $aapt2 link -o (Join-Path $out 'base.apk') -I $plat --manifest (Join-Path $PSScriptRoot 'AndroidManifest.xml') --min-sdk-version 24 --target-sdk-version 34 --version-code 8 --version-name 0.2.5 $resZip
